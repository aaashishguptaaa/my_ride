...
import { ArrowLeft, Bike, Calendar, Car, ChevronRightIcon, IndianRupee, Loader2, MapPin, Phone, Truck, User } from 'lucide-react'
...
function page() {
  ...
  const router=useRouter()
    ...
    return (
        <div className='min-h-screen bg-gray-50 pt-20 sm:pt-24 relative overflow-hidden'>
            {/* Pure CSS Ambient Background Glow - 0% CPU cost */}
            <div className="fixed inset-0 pointer-events-none -z-10 overflow-hidden">
                <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[350px] bg-blue-500/5 rounded-full blur-[140px]" />
                <div className="absolute bottom-10 right-10 w-[500px] h-[300px] bg-emerald-500/5 rounded-full blur-[140px]" />
            </div>

            <div className='bg-white/90 backdrop-blur-md border-b border-gray-200/80'>
                <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
                    <div className='max-w-3xl mx-auto py-6'>
                        <div className='flex items-center gap-4'>
                            <button
                                onClick={() => router.push('/')}
                                className="w-10 h-10 rounded-xl bg-white border border-gray-200 shadow-sm flex items-center justify-center hover:bg-gray-50 text-gray-700 transition-colors cursor-pointer"
                                title="Back to Home"
                            >
                                <ArrowLeft size={17} />
                            </button>
            ...
            <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6'>
                <div className='max-w-3xl mx-auto space-y-6'>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div className="p-3.5 rounded-2xl bg-white border border-gray-200/80 shadow-xs text-center">
                            <span className="text-[10px] uppercase font-bold text-gray-400 block tracking-wider">Completed</span>
                            <span className="text-xl font-extrabold text-emerald-600">
                                {bookings.filter(b => b.bookingStatus === 'completed').length} Rides
                            </span>
                        </div>
                        <div className="p-3.5 rounded-2xl bg-white border border-gray-200/80 shadow-xs text-center">
                            <span className="text-[10px] uppercase font-bold text-gray-400 block tracking-wider">Total Booked</span>
                            <span className="text-xl font-extrabold text-gray-900">
                                {bookings.length}
                            </span>
                        </div>
                        <div className="p-3.5 rounded-2xl bg-white border border-gray-200/80 shadow-xs text-center">
                            <span className="text-[10px] uppercase font-bold text-gray-400 block tracking-wider">Lifetime Spent</span>
                            <span className="text-xl font-extrabold text-blue-600">
                                ₹{bookings.filter(b => b.bookingStatus === 'completed').reduce((sum, b) => sum + (b.fare || 0), 0).toLocaleString()}
                            </span>
                        </div>
                    </div>
                    ...
                    {!loading && filterBookings.length > 0 && (
                        <div className='space-y-4'>
                            {filterBookings.map((b, i) => (
                                ...
                                >
                                    <div className='bg-white rounded-2xl border border-gray-200/90 shadow-sm hover:shadow-md hover:border-gray-300 transition-all overflow-hidden'>
