...
import { ArrowLeft, ArrowRight, Banknote, Bike, Car, CheckCircle, Clock, CreditCard, IndianRupee, Loader2, MapPin, Navigation, Shield, Truck, Wallet, XCircle } from 'lucide-react';
...
function CheckOutContent() {
  ...
  const handleRequestBooking = async () => {
    ...
    useEffect(() => {
       ...
       socket.on("accept-booking", (data) => {
         if (data === "confirmed" || (typeof data === "object" && data?.status === "confirmed")) {
           setStatus("confirmed")
         } else {
           setStatus(data)
         }
    ...
    useEffect(() => {
      if (status !== "confirmed" || !booking?._id) return;
  ...
  return (
    <div className='min-h-screen bg-zinc-100 px-4 pt-28 pb-16'>
      <div className='relative max-w-6xl mx-auto z-10'>
        ...
        >
          <motion.button
            ...
            onClick={() => router.back()}
            className="w-11 h-11 rounded-2xl bg-white border border-zinc-200 shadow-sm flex items-center justify-center hover:bg-zinc-50 hover:border-zinc-300 transition-all flex-shrink-0 cursor-pointer"
            title="Go Back"
          >
          ...
          <div>
            ...
            <p className='text-zinc-400 text-xs sm:text-sm mt-0.5 font-medium'>Review your ride and confirm</p>
          </div>
        </motion.div>
        <div className='grid lg:grid-cols-2 gap-6'>
          ...
          <motion.div
            ...
            className="bg-white rounded-3xl border border-zinc-200 overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.07)] flex flex-col relative"
          >
                  ...
                  >
                    <div>
                      ...
                      <div className='bg-zinc-50 border border-zinc-100 rounded-2xl p-5 space-y-3'>
                        {
                          [
                            { icon: <Clock size={14} />, text: "Driver responds and heads to pickup immediately" },
                            { icon: <Shield size={14} />, text: "Verified & background-checked drivers only" },
                            { icon: <CreditCard size={14} />, text: "No advance needed: pay at destination or pickup (Cash / UPI)" },
                          ].map((item, i) => (
                            <div key={i} className="flex items-center gap-3">
                              ...
                              <p className='text-zinc-600 text-xs font-semibold'>{item.text}</p>
                            </div>
                          ))
                        }
                      </div>
                    ...
                    <motion.button
                      ...
                      disabled={loading}
                      className="w-full h-14 mt-8 bg-zinc-950 hover:bg-black disabled:opacity-60 text-white font-black text-sm rounded-2xl flex items-center justify-center gap-2.5 transition-all shadow-xl shadow-zinc-950/20 cursor-pointer group"
                    >
                      {loading ? (
