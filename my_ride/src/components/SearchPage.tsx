        ...
        <div className='min-h-screen bg-zinc-100 text-zinc-900 overflow-x-hidden'>
            <div className='fixed top-24 left-5 z-50'>
                <motion.button
                    ...
                    onClick={() => router.back()}
                    title="Go Back"
                    className="w-11 h-11 rounded-2xl bg-white/95 backdrop-blur-md border border-zinc-200 shadow-lg flex items-center justify-center hover:bg-zinc-100 text-zinc-900 transition-all cursor-pointer"
                >
                    <ArrowLeft size={18} />
                </motion.button>
            </div>
            ...
            <motion.div
                ...
                className="relative z-20 -mt-10 bg-white/95 backdrop-blur-xl rounded-t-[32px] border-t border-zinc-200/80 shadow-[0_-8px_40px_rgba(0,0,0,0.08)] pt-4 pb-20 min-h-[52vh]"
            >
                {/* Drag Handle & Rim */}
                ...
                <div className='px-5 lg:px-8 max-w-6xl mx-auto'>
                    <motion.div
                        ...
                        className="bg-zinc-50 border border-zinc-200/90 rounded-2xl overflow-hidden mb-5 shadow-xs"
                    >
                        <div className='flex gap-3 px-4 py-3 border-b border-zinc-100'>
                            ...
                            <MapPin size={14} className="text-emerald-500 flex-shrink-0 mt-1.5" />
                        </div>
                        <div className='flex gap-3 px-4 py-3'>
                            <div className='flex flex-col items-center pt-1.5 flex-shrink-0'>
                                <div className='w-2.5 h-2.5 rounded-full bg-sky-500 ring-2 ring-sky-500/20' />
                            </div>
                            ...
                            <Navigation size={14} className="text-sky-500 flex-shrink-0 mt-1.5" />
                        </div>

                        {km > 0 && (
                            <div className="px-4 py-2 bg-gradient-to-r from-emerald-500/10 to-sky-500/10 border-t border-zinc-100 flex items-center justify-between text-xs font-bold text-zinc-700">
                                <span className="flex items-center gap-1.5">
                                    <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                                    <span>Route Distance</span>
                                </span>
                                <span className="font-mono text-zinc-900 bg-white px-2 py-0.5 rounded-lg border border-zinc-200/80 shadow-xs">
                                    {km} km • ~{Math.round(km * 2.8 + 3)} min
                                </span>
                            </div>
                        )}
                    ...
                    <AnimatePresence>
                        {!loading && vehicles.length === 0 && (
                            ...
                            >
                                ...
                                <p className='text-zinc-400 text-sm max-w-xs leading-relaxed'>No {meta?.label || "vehicle"} drivers are currently available near your pickup location.</p>
                                <motion.button
                                    whileTap={{ scale: 0.95 }}
