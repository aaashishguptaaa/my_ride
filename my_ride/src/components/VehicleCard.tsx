...
function VehicleCard({ vehicle, distance, onBook }: { vehicle: IVehicle, distance: number | undefined, onBook: () => void }) {
    const { Icon, label } = TYPE_CONFIG[vehicle.type] || { Icon: Car, label: "Vehicle" }
    const base = vehicle.baseFare || 30
    const perKm = vehicle.pricePerKM || 10
    const estimated = Math.round(base + (perKm * (distance && distance > 0 ? distance : 2.5)))
            ...
            <div className='relative h-48 bg-zinc-50 flex items-center justify-center overflow-hidden'>
                ...
                {vehicle.imageUrl ? (
                    ...
                    <div className="relative z-10 flex flex-col items-center justify-center p-4">
                        <div className="w-20 h-20 rounded-2xl bg-zinc-900 text-white flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform">
