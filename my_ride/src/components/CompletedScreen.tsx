...
import { ArrowLeft, CheckCircle2, IndianRupee, User, Zap } from 'lucide-react'
     ...
     <div >
        ...
        <p className='text-white text-sm font-bold'>{booking?.user?.name??"user"}</p>
                    ...
                    {role === "driver" ? (
                        ...
                        <button
                            onClick={() => router.push("/")}
                            className="w-full bg-white hover:bg-zinc-200 text-zinc-950 py-3.5 rounded-xl text-sm font-bold transition shadow-lg flex items-center justify-center gap-2 cursor-pointer"
                        >
